...
function ResumeThumbnail({ resume }) {
  ...
  const thumbnailResume = {
    ...resume,
    personal_info: resume.personal_info || { full_name: resume.name || "Your Name" },
    education: resume.education || [],
    experience: resume.experience || [],
    skills: resume.skills || [],
    projects: resume.projects || [],
  };
  if (!thumbnailResume.personal_info.full_name && resume.name) {
  ...
  return (
    <div className="resume-card-preview" aria-hidden="true">
      <div className="resume-card-preview-scale">
        <ResumeDocument resume={thumbnailResume} templateId={templateId} />
      </div>
        ...
        <section className="dashboard-section">
          <div className="section-heading-row">
            ...
            {!loading && resumes.length > 0 && (
              <span className="resume-count">{resumes.length} {resumes.length === 1 ? "resume" : "resumes"}</span>
            )}
          </div>
              ...
              {resumes.map((resume) => (
                <article className="resume-card" key={resume.id}>
                  ...
                  <div className="resume-card-top">
                    ...
                    <button type="button" className="card-menu" onClick={() => handleDelete(resume.id, resume.name)} title="Delete resume" aria-label={`Delete ${resume.name}`}>
                      ⋮
                    </button>
                  </div>
                  ...
                  <div className="resume-card-content">
                    ...
                    <div className="resume-card-meta-row">
                      ...
                      <Link className="template-pill-link" to={`/resumes/${resume.id}/template`} title="Change template">
