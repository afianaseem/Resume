      ...
      {TEMPLATES.map((tpl) => {
        ...
        return <article key={tpl.id} className={`template-card ${selected ? "selected" : ""}`}>
          <button type="button" className="template-preview-button" onClick={() => setSelectedTemplate(tpl.id)} aria-label={`Select ${tpl.name}`}>
            <div className="template-real-preview"><div className="template-preview-viewport"><div className="template-preview-page"><ResumeDocument resume={{ ...resume, color: selected ? selectedColor : (resume.color || "violet") }} templateId={tpl.id} /></div></div><span className="template-preview-label">Click to select</span></div>
          </button>
