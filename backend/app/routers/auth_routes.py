...
def signup(payload: schemas.UserSignup, db: Session = Depends(get_db)):
    try:
        ...
        db.rollback()
        print(f"[auth/signup] database error: {exc!r}")
        raise HTTPException(
            status_code=503,
...
@router.post("/logout")
def logout():
    # JWTs are stateless: logging out is handled client-side by deleting the
    # stored token. This endpoint exists so the frontend has a clean call to make.
