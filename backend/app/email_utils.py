...
def _email_message_html(text: str) -> str:
    """
    Convert the user's multiline message into safe HTML while preserving
    ...
    Multiple empty lines are also preserved.
    """
    if text is None:
        return ""
    ...
    # Escape HTML so user-entered content cannot inject HTML.
    escaped = html.escape(normalized)

    # Preserve every newline, including consecutive newlines.
...
def build_resume_pdf(resume: Dict[str, Any]) -> bytes:
    ...
    palettes = {
        "violet": ("#6d3df5", "#5730d0", "#f0ebff"),
        "blue": ("#2563eb", "#1d4ed8", "#dbeafe"),
        "green": ("#059669", "#047857", "#d1fae5"),
        "rose": ("#e11d48", "#be123c", "#ffe4e6"),
    }

    accent, accent_dark, accent_light = palettes.get(
        resume.get("color") or "violet",
        palettes["violet"],
    )
    ...
    def sections(story):
        if pi.get("summary"):
            story += [
                ...
                Paragraph(_safe(pi["summary"]), body),
            ]
        ...
        if experience:
            ...
            for exp in experience:
                role = _safe(exp.get("role") or "Role")

                if exp.get("company"):
                    role += f" — {_safe(exp.get('company'))}"

                story.append(Paragraph(role, item))

                if _date_range(exp):
                    story.append(
                        Paragraph(
                            _safe(_date_range(exp)),
                            small,
                        )
                    )
        ...
        if projects:
            ...
            for proj in projects:
                story.append(
                    Paragraph(
                        _safe(proj.get("title") or "Project"),
                        item,
                    )
                )
                ...
                if proj.get("description"):
                    story.append(
                        Paragraph(
                            _safe(proj["description"]),
                            body,
                        )
                    )

    full_name = _safe(
        pi.get("full_name") or "Your Name"
    )
    ...
    if template == "sidebar":
        ...
        if template == "modern":
            ...
            story += [
                Paragraph(full_name, title),
                (
                    Paragraph(contacts, small)
                    if contacts
                    else Spacer(1, 1)
                ),
            ]
...
def send_resume_email(
    ...
    The user's custom note is sent as HTML, but all line breaks are preserved.
    This means:

        Best Regards,
        XYZ

    remains exactly two lines in the received email.

    Blank lines are also preserved.
    """
    ...
    subject = (
        f"{sender_name} shared their resume: {resume_name}"
        if sender_name
        else f"Your resume: {resume_name}"
    )
    ...
    intro = (
        ...
        if note is not None and note != ""
            ...
            if sender_name
            else "Here is the resume you requested from ResumeForge."
        )
    )

    # IMPORTANT:
    # Convert newlines to <br> AFTER escaping HTML.
    # This preserves line breaks without allowing HTML injection.
    intro_html = _email_message_html(intro)
    ...
    html_body = (
        ...
        f'<p>'
        f'The resume '
        f'<strong>{html.escape(resume_name)}</strong> '
        ...
        'margin-top:20px;'
        '">'
        'Sent from ResumeForge.'
        '</p>'
        '</div>'
    )

    # Plain-text fallback.
    # This is important because some email clients prefer plain text.
    plain_body = (
        f"{intro}\n\n"
        f"The resume {resume_name} is attached as a PDF.\n\n"
        "Sent from ResumeForge."
    )
